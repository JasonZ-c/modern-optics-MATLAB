function iEout = iRS(x,y,iEi,xo,yo,wavelength,z) %xo,yo=output
k=2*pi/wavelength;
N=length(x);
M=length(xo);
dx=abs(x(2)-x(1));
...
U=zeros(M+N-1);
U((1:N)-round(N/2)+round((N+M-1)/2),(1:N)-round(N/2)+round((N+M-1)/2))=iEi;%iEi at the centre of U, as the output of RS

e1=(fft2(U)); 
e2=(fft2((H)));%FFT 2demensions
e=fftshift(ifft2((e1.*e2)))*dx^2;%fftshift modify fft2's side effect
iEout=e(-1+(1:M)-1*round(M/2)+1*(round((M+N-1)/2)),-1+(1:M)-1*round(M/2)+1*round((N+M-1)/2));
end